...
// フィールドサイズ
const FILED_W = SCREEN_W +120;
const FILED_H = SCREEN_H +40;
...
function gameInit()
{
    ...
    // requestAnimationFrameにするとより正確なfpsを出すことができる。
}
...
function drawAll()
{
    ...
    camera_x = Math.floor((jiki.x>>8)/FILED_W * (FILED_W-SCREEN_W));
    camera_y = Math.floor((jiki.y>>8)/FILED_H * (FILED_H-SCREEN_H));
...
function gameLoop()
{
    ...
    if( gameWave == 0 )
    {
        ...
        {
            // let r = rand(0,1);
            teki.push( new Teki( 0,rand(0,FILED_W)<<8 ,0, 0, rand(300,1200) ) );
        }
        if( gameCount  >60*20)
        {
            gameWave++;
    ...
    else if( gameWave == 1 )
    {
        ...
        {
            // let r = rand(0,1);
            teki.push( new Teki( 1,rand(0,FILED_W)<<8 ,0, 0, rand(300,1200) ) );
        }
        if( gameCount  >60*20)
        {
            gameWave++;
    ...
    else if( gameWave == 2 )
    {
        ...
        {
            let r = rand(0,1);
            teki.push( new Teki( r,rand(0,FILED_W)<<8 ,0, 0, rand(300,1200) ) );
        }
        if( gameCount  >60*20)
        {
            gameWave++;
            gameCount=0;
            teki.push( new Teki( 2,(FILED_W/2)<<8 ,-(70<<8), 0, 200 ) );
            starSpeedReq+600
        }
    }
    else if( gameWave == 3 )
...
{
    gameInit();
    // teki.push( new Teki( 2,(FILED_W/2)<<8 ,0, 0, 200 ) );
}